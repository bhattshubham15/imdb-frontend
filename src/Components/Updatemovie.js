...
import { addGenre, updateMovie, getMovieView } from '../Services';
...
export default class Updatemovie extends Component {
    ...
    async componentDidMount() {
        ...
        if (token) {
            if (this.props.location.id) {
                sessionStorage.setItem('locationId', this.props.location.id)
            }
            const locationId = sessionStorage.getItem('locationId');
            const getView = await getMovieView(token, locationId);
            console.log(getView);
            if (getView && getView.data) {
                this.setState({
                    token: token,
                    name: getView.data.name,
                    director: getView.data.director,
                    popularity: getView.data.popularity,
                    imdb_score: getView.data.imdb_score,
                    genre_name: '',
                })
            }
        }
    ...
    handleSubmit = async (e) => {
        ...
        if (this.handleValidation()) {
            ...
            const addMovie = await updateMovie(data, token, this.props.location.id);
            if (addMovie) {
                this.setState({
                    name: '',
                    director: '',
                ...
                <div className="content-container">
                    <div className="form-container">
                        <form className="row" onSubmit={this.handleSubmit}>
                            <h1>
                                EDIT MOVIE DETAILS HERE
                                </h1>
                            <div>
                                <span className="subtitle">MOVIE NAME:</span>
                                <span>
                                    <input disabled={true} type="text" name="name" id="name" value={this.state.name} onChange={this.handleChange} onBlur={this.handleBlur} />
                                </span>
